...
function insertNumber(e) {
    ...
    if (code == 39) { // Right arrow
        if (col < 8) {
            nextObject = document.getElementById('square_' + row + '_'
                    + (col / 1 + 1));
    ...
    if (code == 38) {
        if (row > 0) {
            nextObject = document.getElementById('square_' + (row - 1) + '_'
                    + col);
            if (nextObject.style.backgroundColor) {
...
function getPossibleNumbers(inputObj) {
    ...
    for (var no = 0; no < subDivs.length; no++) {
        if (subDivs[no] != inputObj) {
            var spans = subDivs[no].getElementsByTagName('SPAN');
            if (spans[0].innerHTML == spans[1].innerHTML
                    || subDivs[no].style.backgroundColor.length > 1) {
...
function switchLevel(initLevel, linkObj) {
    ...
    if (!confirmSwitch)
        confirmSwitch = confirm('Click OK to finish the current game');
    if (confirmSwitch) {
